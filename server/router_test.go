...
func TestHTMLPreviewRuntimeUsesRenderedThreadContract(t *testing.T) {
	...
	for _, want := range []string{
		...
		`drafting-rendered-comment:not(tr)::before,tr.vivi-rendered-comment-block.drafting-rendered-comment{background:var(--comment-tint);box-shadow:inset 2px 0 0 var(--comment-line);}`,
		`active-rendered-comment:not(tr)::before,tr.vivi-rendered-comment-block.active-rendered-comment{background:var(--comment-tint);box-shadow:inset 2px 0 0 var(--comment-text);}`,
	...
	for _, unwanted := range []string{
		`z-index:-1`,
		`.has-rendered-comment:not(tr)::before,.vivi-rendered-comment-block.drafting-rendered-comment`,
