...
`POST /graphql` is the normal SPA data path. Requests use JSON GraphQL
...
generated by `gqlgen`. Regenerate the Go GraphQL transport with
`task generate:graphql` after schema changes. The schema models Vivi concepts
