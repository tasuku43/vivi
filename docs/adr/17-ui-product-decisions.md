...
- Queued documents should be projected into their real path hierarchy and reuse the left Explorer's indentation, icons, collapse, selection, and keyboard language. Every queue-bearing directory is independently collapsible; only ancestors of queued documents appear, so the projection does not become a second full repository tree.
- Directory grouping is a navigation projection, not queue priority. “Next queued” and keyboard next/previous actions continue to follow the globally ordered queue even when a directory containing that stop is collapsed.
- `In Review` remains immediately below the queued directory projection and contains the workspace-level `Ready to publish` surface. Grouping the waiting queue must not separate pending drafts from the publish workflow.
