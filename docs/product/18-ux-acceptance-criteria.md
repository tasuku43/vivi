...
16. Keep browser-local comment input across outside clicks, document/tab navigation, rendered/source changes, and reload. Explicit Discard removes it. In rendered Markdown and HTML preview, saving the first comment promotes it to a durable pending thread, clears the same composer into an empty additional-note input, and keeps that input focused. Saving later notes likewise clears the submitted body and preserves focus for consecutive review thoughts. Publishing saved drafts must not discard a next unsaved thought at the same anchor.
