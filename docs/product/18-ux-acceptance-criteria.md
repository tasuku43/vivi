...
19. Scan active Markdown and HTML review files in one compact signal ledger, filter them by All, Unread, Drafts, or Changed, and retain each parent path plus exact per-file diff totals. Change-only files recede thirty minutes after the latest watcher event or user open, while pending drafts and files with any non-archived feedback remain. Remove feedback-only files after every thread is archived. Keep draft publication on the actionable file row while global “Next queued” order remains unchanged.
