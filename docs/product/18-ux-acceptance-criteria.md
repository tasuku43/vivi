...
18. Move between For you and Document through two persistent inspector tabs whose labels and positions stay fixed; keep For you selected by default.
19. Scan **For you** without choosing a filter: **Feedback** keeps open unread human feedback, drafts, and in-progress input; **Recent** shows meaningful document activity for thirty minutes. Git startup discovery, content changes, explicit opens, agent presentations, and the first read of new feedback share that window. Passive refresh, restoration, unchanged saves, and repeated reads do not extend it. At thirty minutes recent rows leave while open tabs remain. Hide for now suppresses replay until new activity. Rows retain filename, H1, reason, relative time, and collision-disambiguating parent paths; feedback publication remains on its row. Pointer/focus interaction holds ordering stable. Missing and excluded sources leave active counts and remain under collapsed Unavailable feedback. Archived/resolved feedback does not count as unread. Servers using the same workspace data directory synchronize activity and receipts.
...
- For you shows each document once, under Feedback or Recent. Its count appears
  only in the tab; filters and Next queued are absent. Product UI copy is English.
