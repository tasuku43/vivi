...
func TestCommentsCLIClassifiesServerUnreachableForAgentRecovery(t *testing.T) {
	...
	if len(envelope.Error.SuggestedCommands) != 2 || envelope.Error.SuggestedCommands[0].Command != "comments protocol" || !containsString(envelope.Error.SuggestedCommands[0].Args, receiptLog) || envelope.Error.SuggestedCommands[1].Command != "comments doctor" || !containsString(envelope.Error.SuggestedCommands[1].Args, "doctor-start-1") || !containsString(envelope.Error.SuggestedCommands[1].Args, "http://127.0.0.1:4317") || !containsString(envelope.Error.SuggestedCommands[1].Args, receiptLog) {
...
func TestCommentsCLISchemaSurfacesStructuredStdinContracts(t *testing.T) {
	...
	if inboxExampleSummary["recommendedAction"] != "resume_owned_work" || inboxPayload.Example["mine"].(map[string]any)["count"] != float64(1) && inboxPayload.Example["mine"].(map[string]any)["count"] != 1 {
