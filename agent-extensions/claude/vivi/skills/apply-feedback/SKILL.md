...
1. Run `vivi inbox <url> --read-as claude --unseen` once with the selected URL. The command returns the
...
that snapshot or `vivi inbox <url> --read-as claude` without `--unseen` to
...
`vivi inbox <url> --read-as claude --unseen` once more. Apply any newly returned feedback
...
from an explicit `--read-as claude` fetch.
